...
6. 审阅安全报告，将问题提交工程师组长安排修复
7. **不参与开发细节** — 开发任务的管理、调度、分支、合并全部交由工程师组长执行
...
4. **问题分类**: 收到用户输入后，首先判断属于以下哪一类：
   ...
   | 类别 | 判定标准 | 处理路径 |
   |------|----------|----------|
   | 🐛 **Bug** | 现有功能异常、报错、不符合预期行为、回归问题 | 启动工程师组长调度工程师修复 |
...
Bug 不经过需求分析和技术设计，技术主管直接调度工程师组长，由组长安排对应工程师排查修复。Bug 的完整报告写入 `docs/smart-dev/bug-log.md`，作为可追溯的文档记录：
...
3. **启动工程师组长 Agent**: 使用 Agent 工具，传递：
   - Bug 描述（用户报告的完整现象、错误信息、复现步骤）
   - 要求组长评估 Bug 类型和所需工程师类型，调度对应工程师进行根因分析和修复
   - 要求工程师修复后，组长补充完整报告（根因、修复方式、影响范围）到 `docs/smart-dev/bug-log.md`
   - 修复后组长向技术主管汇报
4. 等待工程师组长返回修复完成报告
5. **审阅 Bug 报告**: 确认 `docs/smart-dev/bug-log.md` 内容完整，包含现象、根因、修复方式、影响范围
6. 更新状态 `"phase": "bugfix", "status": "completed"`
7. 向用户汇报修复结果，并提示可查看 `docs/smart-dev/bug-log.md` 查阅完整记录
...
> 所有级别的任务都执行本阶段。**简单任务同样需要启动工程师组长**，组长评估后安排对应工程师完成编码，**组长本人不参与编码**，不得由技术主管或工程师组长直接编码。
...
2. **启动工程师组长 Agent**: 使用 Agent 工具，传递：
   ...
   - **要求**:
     ...
     - 调度对应工程师完成任务
     - 生成 `docs/smart-dev/implementation-report.md`，记录每项任务由哪位工程师实现及完成时间
     - 确保 Bug 记录（标注修复工程师）和架构问题升级
     - 遇到无法解决的卡住问题时及时上报
     - 开发完成后汇报
3. 等待工程师组长返回开发完成报告（或终止通知）
4. **处理工程师组长上报的问题**:
   - 收到**卡住报告**: 评估问题原因，给出解决方案或调整任务安排，回复工程师组长继续执行
   - 收到**架构问题升级**: 评估并判定解决方案，通知工程师组长执行
5. 更新状态
6. **向用户汇报**: 展示完成的功能清单、代码变更概要，提示可查看 `docs/smart-dev/implementation-report.md` 了解每位工程师的实现记录
...
3. 审阅安全报告：
   - **无风险或低风险**: 进入步骤 4
   - **有中/高风险**: 将安全报告提交给**工程师组长**，由组长将修复项作为新任务纳入开发排期，调度工程师修复。修复完成后重新执行步骤 2
4. 安全专家确认安全性后：
