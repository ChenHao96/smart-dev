...
一个 Claude Code 插件，由8个 Skill 组成的虚拟开发团队（7个角色 + 1个通用规范），覆盖从需求分析到生产部署的完整开发生命周期。
...
| 📐 **工程师通用规范** | — | 所有工程师共享的核心原则、开发流程、质量要求 |
| 💻 **前端工程师** | `/frontend` | Web/移动端 UI 开发、组件架构、交互实现 |
| ⚙️ **后端工程师** | `/backend` | 服务端 API、数据库、业务逻辑、系统集成 |
| 🔧 **硬件工程师** | `/hardware` | 嵌入式固件、驱动开发、硬件调试与集成 |
...
### 阶段3: 编码开发
技术主管评估任务类型、制定排期，直接调度前端/后端/硬件工程师，每个工程师从 dev 创建 `feature/<任务名>` 分支独立开发，按检查点汇报，技术主管审查结果并批准合并回 dev。
